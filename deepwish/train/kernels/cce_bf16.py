...
def _cce_fwd_parallel_kernel(
    ...
    #     accum += e_d[:, None] * c_d[None, :]
    # Convert accum to bf16 for storage efficiency
...
class TritonCCE(torch.autograd.Function):
    ...
    def forward(ctx, E, C, x, block_size=128, block_n=16, num_locks=128):
        # E: [N,D], C: [V,D], x: [N] - Ensure bf16
        E = E.to(torch.bfloat16) if E.dtype != torch.bfloat16 else E
        C = C.to(torch.bfloat16) if C.dtype != torch.bfloat16 else C
        assert E.dim() == 2 and C.dim() == 2
        N, D = E.shape
        V, D_ = C.shape
        assert D == D_
        # allocate intermediates
        lse = torch.full((N,), -float('inf'), dtype=torch.float32, device=E.device)
        logits_corr = torch.zeros((N,), dtype=torch.float32, device=E.device)
    ...
    def backward(ctx, grad_loss):
        ...
        V, _ = C.shape
        # alloc grads
        # unfortunately, atomic add is not supported with bf16 on cuda 8.9 < 9.
        ...
        return dE.to(torch.bfloat16), dC.to(torch.bfloat16), None, None
...
def test_forward():
    ...
    E = torch.randn(N, D, device=device, dtype=torch.bfloat16, requires_grad=True)
    C = torch.randn(V, D, device=device, dtype=torch.bfloat16, requires_grad=True)
    x = torch.randint(0, V, (N,), device=device, dtype=torch.long)
    # reference loss
    logits = E @ C.t()
    loss_fn = torch.nn.CrossEntropyLoss(reduction="mean")
    ref_loss = loss_fn(logits, x).to(torch.bfloat16)
    # triton loss
    tri_loss = triton_cut_cross_entropy(E, C, x, block_size).to(torch.bfloat16)
...
def benchmark(V, provider, N, D, block_size):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    E = torch.randn(N, D, device=device, dtype=torch.bfloat16)
    C = torch.randn(V, D, device=device, dtype=torch.bfloat16)
    x = torch.randint(0, V, (N,), device=device, dtype=torch.long)
    loss_fn = torch.nn.CrossEntropyLoss(reduction="mean")
    if provider == "Torch":
