...
def _attn_bwd_dq(dq, q, K, V, O, do, m, stride_tok, stride_d, H, N_CTX, \
    ...
    for blk_idx in range(num_steps):
        ...
        ds = p * (dp - delta[:, None])
        dq += tl.dot(ds, tl.trans(kT).to(tl.float32)).to(tl.bfloat16)
        curr_n += step_n
...
class _attention(torch.autograd.Function):
    @staticmethod
    def forward(ctx, q, k, v, sm_scale): # q,k,v bf16 -> o bf16
    ...
    @staticmethod
    def backward(ctx, do): # do is bf16 as well here
        q, k, v, o, M = ctx.saved_tensors
        # ensure gradient input is contiguous and bf16
        do = do.to(torch.bfloat16) if do.dtype != torch.bfloat16 else do
        if not do.is_contiguous():
            do = do.contiguous()
        dq = torch.empty_like(q, dtype=torch.bfloat16)
        dk = torch.empty_like(k, dtype=torch.bfloat16)
        dv = torch.empty_like(v, dtype=torch.bfloat16)
...
def test_forward():
    ...
    q = torch.randn((batch_size, n_heads, n_ctx, head_dim), dtype=torch.bfloat16, device=DEVICE, requires_grad=True)
    k = torch.randn((batch_size, n_heads, n_ctx, head_dim), dtype=torch.bfloat16, device=DEVICE, requires_grad=True)
    v = torch.randn((batch_size, n_heads, n_ctx, head_dim), dtype=torch.bfloat16, device=DEVICE, requires_grad=True)
    ...
    close_elements = torch.isclose(ref_out, attn_out, atol=0.1, rtol=0).sum().item()
    total_elements = ref_out.numel()
    percentage_correct = (close_elements / total_elements) * 100
    print(f"Forward test: {percentage_correct:.2f}% of elements are within 0.1 (atol) of PyTorch output.")
    assert percentage_correct >= 90.0, f"Forward test failed: {percentage_correct:.2f}% elements close, expected 95%"
...
def test_backward():
    ...
    close_elements_dq = torch.isclose(ref_dq, tri_dq, atol=0.1, rtol=0).sum().item()
    total_elements_dq = ref_dq.numel()
    percentage_correct_dq = (close_elements_dq / total_elements_dq) * 100
    print(f"Backward test (dQ): {percentage_correct_dq:.2f}% of elements are within 0.1 (atol) of PyTorch output.")
    assert percentage_correct_dq >= 95.0, f"Backward test (dQ) failed: {percentage_correct_dq:.2f}% elements close, expected 95%"

    close_elements_dk = torch.isclose(ref_dk, tri_dk, atol=0.1, rtol=0).sum().item()
    total_elements_dk = ref_dk.numel()
    percentage_correct_dk = (close_elements_dk / total_elements_dk) * 100
    print(f"Backward test (dK): {percentage_correct_dk:.2f}% of elements are within 0.1 (atol) of PyTorch output.")
    assert percentage_correct_dk >= 95.0, f"Backward test (dK) failed: {percentage_correct_dk:.2f}% elements close, expected 95%"

    close_elements_dv = torch.isclose(ref_dv, tri_dv, atol=0.1, rtol=0).sum().item()
    total_elements_dv = ref_dv.numel()
    percentage_correct_dv = (close_elements_dv / total_elements_dv) * 100
    print(f"Backward test (dV): {percentage_correct_dv:.2f}% of elements are within 0.1 (atol) of PyTorch output.")
...
if __name__ == "__main__":
    ...
    def run_torch_forward():
        ...
        p = p.masked_fill(tril == 0, float("-inf"))
        p = torch.softmax(p.float(), dim=-1).to(torch.bfloat16)
        return torch.matmul(p, v.to(torch.bfloat16))
