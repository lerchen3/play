...
class MLA(BaseAttention):
    ...
    def forward(self, h, cached_c_kv=None, cached_kR=None, cached_len=0, return_latent=False):
        ...
        # Compose K and V
        k = torch.cat((ku, kR_full), dim=-1).permute(0, 2, 1, 3).contiguous()  # (B, H, T, 2*dh)
        v = vu.permute(0, 2, 1, 3).contiguous()  # (B, H, T, 2*dh)
        ...
        if cached_len > 0 and (
            ...
            q_decode = q_suffix.permute(0, 2, 1, 3).contiguous()
            attn_suffix = single_query_attention(q_decode, k, v, self.scale)
            attn_suffix = attn_suffix.permute(0, 2, 1, 3).reshape(B, S, -1)
        else:
            if cached_len > 0:
                q_prefix = torch.zeros((B, cached_len, self.n_heads, 2 * self.d_head), device=h.device, dtype=q_suffix.dtype)
                q_full = torch.cat([q_prefix, q_suffix], dim=1).permute(0, 2, 1, 3)
            attn = self.compute_attention(q_full, k, v, is_decoding)
            attn = attn.permute(0, 2, 1, 3).reshape(B, T, -1)
            attn_suffix = attn[:, -S:, :] if cached_len > 0 else attn

        out_suffix = attn_suffix @ self.W_o
        
        # Final validation of output
        if not torch.isfinite(out_suffix).all():
            print("Warning: NaN/Inf in final MLA output after W_o projection")
            out_suffix = torch.zeros_like(out_suffix)
