...
class RewardModel(nn.Module):
    ...
    def forward(self, obs: torch.Tensor, action: torch.Tensor) -> torch.Tensor:
        ...
        rewards = self.net(torch.cat((obs, action)))
    ...
    def compute_reward(self, obs: np.ndarray, action: np.ndarray) -> float:
        ...
        obs = np2torch(obs).unsqueeze(0)
        action = np2torch(action).unsqueeze(0)
    ...
    def update(self, batch: Tuple[torch.Tensor]):
        ...
        reward1, reward2 = 0, 0
        for t in range(obs1.shape[1]):
            reward1 += self.forward(obs1[:, t], act1[:, t])
            reward2 += self.forward(obs2[:, t], act2[:, t])
        prob1 = torch.sigmoid(reward1 - reward2)
        prob2 = torch.sigmoid(reward2 - reward1)
        loss = -torch.mean(label * torch.log(prob2) + (1 - label) * torch.log(prob1)) # avoiding confusing library calls
        #######################################################
        #########          END YOUR CODE.          ############
