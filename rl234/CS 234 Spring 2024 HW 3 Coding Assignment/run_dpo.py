...
class ActionSequenceModel(nn.Module):
    ...
    def act(self, obs: np.ndarray) -> np.ndarray:
        ...
        obs_t = np2torch(obs).unsqueeze(0) if obs.ndim == 1 else np2torch(obs)
        dist = self.distribution(obs_t)
        seq = torch.clamp(dist.sample()[0], -1, 1)
        return seq[:self.action_dim].cpu().numpy()
        #######################################################
        #########          END YOUR CODE.          ############
...
class DPO(ActionSequenceModel):
    ...
    def update(
        ...
        advantage = (curr_log_prob_w - ref_log_prob_w) - (curr_log_prob_l - ref_log_prob_l)
        loss = -F.logsigmoid(self.beta * advantage).mean()
        self.optimizer.zero_grad()
        loss.backward()
        self.optimizer.step()
        #######################################################
