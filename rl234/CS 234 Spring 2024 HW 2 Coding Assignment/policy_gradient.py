...
class PolicyGradient(object):
        ...
        for path in paths:
            ...
            for t in range(rewards.shape[0]):
                ...
                for k in range(t, rewards.shape[0]):
                    power = k - t
                    Gt += self.config.gamma**power * rewards[k]
                returns.append(Gt)
            #######################################################
            #########          END YOUR CODE.          ############
    ...
    def update_policy(self, observations, actions, advantages):
        ...
        # Get distribution over actions for all observations
        distribution = self.policy.action_distribution(observations)
        
        # Calculate log probabilities of the actions that were actually taken
        log_probs = distribution.log_prob(actions)
        
        # Policy gradient loss (negative because optimizer minimizes)
        # We multiply by advantages to determine if actions were good or bad
